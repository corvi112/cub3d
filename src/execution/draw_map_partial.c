...
/*   Updated: 2023/10/15 21:16:32 by acomet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3D.h"

static void	draw_map_partial_square_axis_ordinate(t_game *game, t_player *player, t_coor coor)
{
	uint32_t	color;

	coor.x2 = player->px - ((int)player->px % SQUARE_MAP) - (SQUARE_MAP * (MAP_SIZE - 1));
...
void	draw_map_partial_square_axis_first(t_game *game, t_player *player, t_coor coor)
{
	...
	x_max = coor.x2;
	coor.y2 = player->py - ((int)player->py % SQUARE_MAP) - (SQUARE_MAP * (MAP_SIZE - 1));
...
void	draw_map_partial_square_axis_last(t_game *game, t_player *player, t_coor coor)
{
	...
	x_max = coor.x2;
	coor.y1 = player->py - ((int)player->py % SQUARE_MAP) + (SQUARE_MAP * MAP_SIZE);
	if (coor.x1 % SQUARE_MAP)
	{
		draw_map_partial_square_axis_ordinate(game, player, coor);
		coor.x1 = player->px - ((int)player->px % SQUARE_MAP) - (SQUARE_MAP * (MAP_SIZE - 1));
...
void	draw_map_partial_square_ordin_last(t_game *game, t_player *player, t_coor coor)
{
	...
	y_max = coor.y2;
	coor.x1 = player->px - ((int)player->px % SQUARE_MAP) + (SQUARE_MAP * MAP_SIZE);
	coor.y2 = coor.y1 + SQUARE_MAP;
	while (coor.y1 < y_max)
	{
		color = get_square_color(game->map, coor.x1, coor.y1, game);
